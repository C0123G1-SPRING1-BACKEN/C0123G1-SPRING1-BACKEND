...
public class LiquidationRestController {
    ...
    public ResponseEntity<Page<ICustomerDto>> getListCustomer(@PageableDefault(size = 3) Pageable pageable, @RequestParam("name") String name) {
        Page<ICustomerDto> customerDtoPage = customerService.findAllBySearchCustomer(pageable,name);
