...
public interface IProfitRepository extends JpaRepository<Contracts, Long> {
    ...
    @Query(value = "SELECT c.contract_code AS contractCode,\n" +
            ...
            "        WHEN :startDate = '' THEN LPAD(MONTH(c.start_date), 2, '0') LIKE CONCAT('%',LPAD(:endDate,2,'0'), '%')\n" +
            "        WHEN :endDate = '' THEN LPAD(MONTH(c.start_date), 2, '0') LIKE CONCAT('%',LPAD(:startDate,2,'0'), '%')\n" +
